...
static void fill(GGSurface s,int x,int y,int w,int h,uint32_t c){
 ...
 if(x+w>(int)s.width)w=(int)s.width-x; if(y+h>(int)s.height)h=(int)s.height-y;
 if(w<=0||h<=0)return;
 for(int yy=y;yy<y+h;yy++)for(int xx=x;xx<x+w;xx++)gg_platform_put_pixel((unsigned)xx,(unsigned)yy,c);
}
...
void gg_draw_acceptance_frame(GGSurface s){
 ...
 const uint32_t a[5]={0xffd9d9d9u,0xff7f6bb2u,0xffd63232u,0xff3f7fd9u,0xffe6b22eu};
 for(int i=0;i<5;i++){int w=i==2?330:230,h=i==2?450:330,x=960+(i-2)*300-w/2,y=i==2?300:360;fill(s,x-6,y-6,w+12,h+12,a[i]);fill(s,x,y,w,h,0xff171d2au);}
