...
hbldr_log(const char* msg) {
  ...
  fp = fopen("/data/ps5_homebrew_launcher.log", "a");
  if(!fp) {
    return;
  }
...
fakeapp_create_if_missing(void) {
  ...
  if(stat(FAKE_PATH, &info)) {
    hbldr_notify("creating FAKE_PATH");
  ...
  if(stat(FAKE_PATH "/sce_sys/param.json", &info)) {
    hbldr_notify("writing param.json");
  ...
  if(stat(FAKE_PATH "/eboot.bin", &info)) {
    hbldr_notify("copying base eboot");
...
bigapp_launch_thread(void* args) {
  ...
  hbldr_notify("sceSystemServiceLaunchApp");
...
bigapp_launch(uint32_t user_id, char** argv) {
  ...
  if((parent=find_pid("SceSysCore.elf")) < 0) {
    hbldr_notify("SceSysCore not found");
    puts("SceSysCore.elf is not running?");
    return -1;
  }

  hbldr_notify("attaching syscore");
  ...
  pthread_join(trd, 0);
  hbldr_notify("bigapp child exec ok");
...
bigapp_replace(pid_t pid, uint8_t* elf, const char* progname, int stdio,
  ...
  if(!(brkpoint=kernel_dynlib_entry_addr(pid, 0))) {
    hbldr_notify("entry addr failed");
  ...
  if(pt_continue(pid, SIGCONT)) {
    hbldr_notify("continue to breakpoint failed");
    perror("pt_continue");
    return -1;
  }
  ...
  // Execute the ELF
  hbldr_notify("executing target elf");
  if(elfldr_exec(pid, elf)) {
    hbldr_notify("elfldr_exec failed");
    return -1;
  }

  hbldr_notify("replace done");
...
hbldr_launch(const char*cwd, const char* path, int stdio, char** argv,
  ...
  hbldr_notify("launch start");

  if(fakeapp_create_if_missing()) {
    hbldr_notify("fakeapp missing, remounting");
    if(remount_system_ex()) {
      hbldr_notify("remount_system_ex failed");
      perror("remount_system_ex");
      return -1;
    }
    if(fakeapp_create_if_missing()) {
      hbldr_notify("fakeapp create failed");
  ...
  if(sceUserServiceGetForegroundUser(&user_id)) {
    hbldr_notify("foreground user failed");
    perror("sceUserServiceGetForegroundUser");
    return -1;
  }

  hbldr_notify("foreground user ok");

  if((app_id=sceSystemServiceGetAppIdOfRunningBigApp()) > 0) {
    hbldr_notify("killing previous bigapp");
    if(sceSystemServiceKillApp(app_id, -1, 0, 0)) {
      hbldr_notify("kill previous bigapp failed");
  ...
  if(!(elf=fs_readfile(buf, 0))) {
    hbldr_notify("target elf read failed");
    return -1;
  }

  hbldr_notify("target elf read ok");

  if((pid=bigapp_launch(user_id, argv)) < 0) {
    hbldr_notify("bigapp_launch failed");
    return -1;
  }

  hbldr_notify("raising child privileges");
  elfldr_raise_privileges(pid);

  if(bigapp_replace(pid, elf, path, stdio, cwd, envp) < 0) {
    hbldr_notify("bigapp_replace failed");
    pt_detach(pid, SIGKILL);
    pid = -1;
  }
