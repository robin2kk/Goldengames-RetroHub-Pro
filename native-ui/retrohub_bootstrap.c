...
int main(void){char *argv[]={UI_ELF,0};char *envp[]={0};pid_t ui;remove(LOG);logline("Goldengames RetroHub bootstrap start");if(sceUserServiceInitialize(0)){logline("sceUserServiceInitialize failed");return EXIT_FAILURE;}logline("UserService initialized");if(elevate()){logline("privilege elevation failed");sceUserServiceTerminate();return EXIT_FAILURE;}logline("privilege elevation OK");if(access(UI_ELF,F_OK)!=0){logpath("UI ELF missing: ",UI_ELF);sceUserServiceTerminate();return EXIT_FAILURE;}logpath("UI ELF found: ",UI_ELF);remove(REQUEST);logline("calling hbldr_launch for UI");ui=hbldr_launch("/data/goldengames-retrohub",UI_ELF,-1,argv,envp);if(ui<0){logline("UI hbldr_launch failed");sceUserServiceTerminate();return EXIT_FAILURE;}logline("UI hbldr_launch OK");while(alive(ui)){if(access(REQUEST,F_OK)==0){logline("launch request detected");usleep(150000);launch_request();break;}usleep(100000);}logline("bootstrap exit");sceUserServiceTerminate();return EXIT_SUCCESS;}
